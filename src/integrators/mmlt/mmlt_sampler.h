...
#if !defined(__MMLT_SAMPLER_H)
#define __MMLT_SAMPLER_H
 ...
 */
class MMLTSampler : public Sampler {
public:
    // Construct a new MLT sampler
    MMLTSampler(const MMLTConfiguration &conf);

    /**
     * \brief Construct a new sampler, which operates on the
     * same random number generator as \a sampler.
     */
    MMLTSampler(MMLTSampler *sampler);

    /// Unserialize from a binary data stream
    MMLTSampler(Stream *stream, InstanceManager *manager);
