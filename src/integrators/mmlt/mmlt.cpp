...
#include "mmlt_proc.h"
#include "mmlt_sampler.h"

MTS_NAMESPACE_BEGIN

/*!\plugin{MMLT}{Primary Sample Space Metropolis Light Transport}
 * \order{9}
 * \parameters{
 *     \parameter{bidirectional}{\Boolean}{
 *     MMLT works in conjunction with another rendering
 ...
 *         (in other words, it is \emph{not} rendered by MMLT). This
 *         parameter specifies the number of samples allocated to that method. To
 *         force MMLT to be responsible for the direct illumination
 ...
 *     }
 * }
 * Primary Sample Space Metropolis Light Transport (MMLT) is a rendering
 * technique developed by Kelemen et al. \cite{Kelemen2002Simple} which is
 * based on Markov Chain Monte Carlo (MCMC) integration.
 * \renderings{
 *    \vspace{-2mm}
 *    \includegraphics[width=11cm]{images/integrator_MMLT_sketch.pdf}\hfill\,
 *    \vspace{-3mm}
 *    \caption{MMLT piggybacks on a rendering method that can turn points
 ...
 * MMLT actively searches for \emph{relevant} light paths (as is the case
 ...
 * much from MMLT, since the MCMC data management causes additional
 * computational overheads.
 *
 * An interesting aspect of MMLT is that it performs this exploration
 ...
 * the method powerful: in particular, it is possible to use MMLT as a
 ...
 *
 * The MMLT implementation in Mitsuba can operate on top of either a simple
 ...
 * it is preferable to disable this separation so that MMLT is responsible
...
public:
    MMLT(const Properties &props) : Integrator(props) {
        ...
        m_config.luminanceSamples = props.getInteger("luminanceSamples", 10000);//CHANGED 100000
        ...
        /* This parameter can be used to specify the samples per pixel used to
           ...
           MMLT will handle direct illumination as well */
    ...
    MMLT(Stream *stream, InstanceManager *manager)
     : Integrator(stream, manager) {
        m_config = MMLTConfiguration(stream);
        configure();
    }
    ...
    //Now, don't support modification to the sensor.
    bool render_difference(Scene *scene, Scene *scene_modified, const RenderJob *job,
            int sceneResID, int sceneModifiedResID, int sensorResID, int samplerResID) {
        ref<Scheduler> scheduler = Scheduler::getInstance();
        ref<Sensor> sensor = scene->getSensor();
        ref<Sampler> sampler = sensor->getSampler();
        const Film *film = sensor->getFilm();
        const Film *film_modified = scene_modified->getSensor()->getFilm();
        ...
        std::vector<PathSeed> pathSeeds_modified;
        ...
        ref<MMLTProcess> process = new MMLTProcess(job, queue,
                m_config, directImage, pathSeeds);
        std::vector<Float> luminance_for_depths(m_config.maxDepth+1 ,0.0);
        std::vector<Float> luminance_for_depths_modified(m_config.maxDepth+1 ,0.0);
        m_config.luminance = pathSampler->generateSeeds_mmlt_difference(luminanceSamples,
            m_config.workUnits, false, m_config.importanceMap, pathSeeds, pathSeeds_modified,
            m_config.maxDepth, luminance_for_depths, luminance_for_depths_modified);


        std::vector<Float> PDF_b(m_config.maxDepth+1 ,0.0), CDF_b(m_config.maxDepth+1 ,0.0);
        CDF_b[0] = luminance_for_depths[0];
        for (int k = 1; k <= m_config.maxDepth; k++) CDF_b[k] = CDF_b[k - 1] + luminance_for_depths[k];
        for (int k = 0; k <= m_config.maxDepth; k++) {
            PDF_b[k] = luminance_for_depths[k] / CDF_b[m_config.maxDepth];
            CDF_b[k] = CDF_b[k] / CDF_b[m_config.maxDepth];
        }

        process->set_luminance_for_depths(luminance_for_depths);
        process->set_CDF_b(CDF_b);
        process->set_PDF_b(PDF_b);
        ...
        ref<MMLTSampler> mltSampler = new MMLTSampler(m_config);
        ...
        process->bindResource("scene_modified", sceneModifiedResID);
    ...
    bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        ref<MMLTProcess> process = new MMLTProcess(job, queue,
                m_config, directImage, pathSeeds);
        std::vector<Float> luminance_for_depths(m_config.maxDepth+1 ,0.0);
        m_config.luminance = pathSampler->generateSeeds_mmlt(luminanceSamples,
            m_config.workUnits, false, m_config.importanceMap, pathSeeds, 
            m_config.maxDepth, luminance_for_depths);
        std::vector<Float> PDF_b(m_config.maxDepth+1 ,0.0), CDF_b(m_config.maxDepth+1 ,0.0);
        CDF_b[0] = luminance_for_depths[0];
        for (int k = 1; k <= m_config.maxDepth; k++) CDF_b[k] = CDF_b[k - 1] + luminance_for_depths[k];
        for (int k = 0; k <= m_config.maxDepth; k++) {
...
private:
    ...
    MMLTConfiguration m_config;
};

MTS_IMPLEMENT_CLASS_S(MMLT, false, Integrator)
MTS_EXPORT_PLUGIN(MMLT, "Primary Sample Space MLT");
MTS_NAMESPACE_END
