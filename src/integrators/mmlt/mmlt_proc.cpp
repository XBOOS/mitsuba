...
class MMLTRenderer : public WorkProcessor {
public:
    MMLTRenderer(const MMLTConfiguration &conf, const std::vector<Float> luminance_for_depths,
        const std::vector<Float> CDF_b, const std::vector<Float> PDF_b)
        : m_config(conf), luminance_for_depths(luminance_for_depths), CDF_b(CDF_b), PDF_b(PDF_b) {
    }

    MMLTRenderer(Stream *stream, InstanceManager *manager)
    ...
    void prepare() {
        ...
        m_origSampler = static_cast<MMLTSampler *>(getResource("sampler"));
        ...
        m_sensorSampler = new MMLTSampler(m_origSampler);
        m_emitterSampler = new MMLTSampler(m_origSampler);
        m_directSampler = new MMLTSampler(m_origSampler);
    ...
    void process(const WorkUnit *workUnit, WorkResult *workResult, const bool &stop) {
        ...
        m_pathSampler->sampleSplats_mmlt(Point2i(-1), *current, seed.depth);
        result->clear();
        ...
        for (uint64_t mutationCtr=0; mutationCtr<m_config.nMutations && !stop; ++mutationCtr) {
            ...
            m_pathSampler->sampleSplats_mmlt(Point2i(-1), *proposed, seed.depth);
            ...
            if (a > 0) {
                if (m_config.kelemenStyleWeights && !m_config.importanceMap) {
                    ...
                    currentWeight = (seed.depth+2)/PDF_b[seed.depth]*(1 - a) * current->luminance
                        / (current->luminance/luminance_for_depths[seed.depth] + m_config.pLarge);
                    proposedWeight = (seed.depth+2)/PDF_b[seed.depth]*(1 - a) * (a + (largeStep ? 1 : 0)) * proposed->luminance
                        / (proposed->luminance/luminance_for_depths[seed.depth]+ m_config.pLarge);
                } else {
                ...
                if (m_config.kelemenStyleWeights)
                    ...
                    //     / (current->luminance/m_config.luminance + m_config.pLarge);
                    currentWeight = (seed.depth+2)/PDF_b[seed.depth] * current->luminance
                        / (current->luminance/luminance_for_depths[seed.depth] + m_config.pLarge);
                else
                    currentWeight = 1;
                proposedWeight = 0;
    ...
    ref<WorkProcessor> clone() const {
        return new MMLTRenderer(m_config, luminance_for_depths, CDF_b, PDF_b);
    }

    MTS_DECLARE_CLASS()
private:
    MMLTConfiguration m_config;
    std::vector<Float> luminance_for_depths;
    std::vector<Float> CDF_b;
    std::vector<Float> PDF_b;
    ref<Scene> m_scene;
    ref<Sensor> m_sensor;
    ref<Film> m_film;
    ref<PathSampler> m_pathSampler;
    ref<MMLTSampler> m_origSampler;
    ref<MMLTSampler> m_sensorSampler;
    ref<MMLTSampler> m_emitterSampler;
    ref<MMLTSampler> m_directSampler;
...
MMLTProcess::MMLTProcess(const RenderJob *parent, RenderQueue *queue,
    const MMLTConfiguration &conf, const Bitmap *directImage,
...
void MMLTProcess::set_luminance_for_depths(const std::vector<Float> luminance_for_depths){
    m_luminance_for_depths = luminance_for_depths;
}
void MMLTProcess::set_CDF_b(const std::vector<Float> CDF_b){
    m_CDF_b = CDF_b;
}
void MMLTProcess::set_PDF_b(const std::vector<Float> PDF_b){
    m_PDF_b = PDF_b;
}


ref<WorkProcessor> MMLTProcess::createWorkProcessor() const {
    return new MMLTRenderer(m_config, m_luminance_for_depths, m_CDF_b, m_PDF_b);
}

void MMLTProcess::develop() {
...
void MMLTProcess::processResult(const WorkResult *wr, bool cancelled) {
    LockGuard lock(m_resultMutex);
...
ParallelProcess::EStatus MMLTProcess::generateWork(WorkUnit *unit, int worker) {
    int timeout = 0;
...
void MMLTProcess::bindResource(const std::string &name, int id) {
...
MTS_IMPLEMENT_CLASS_S(MMLTRenderer, false, WorkProcessor)
MTS_IMPLEMENT_CLASS(MMLTProcess, false, ParallelProcess)
