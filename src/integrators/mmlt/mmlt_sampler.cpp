...
MMLTSampler::MMLTSampler(const MMLTConfiguration &config) : Sampler(Properties()) {
    m_random = new Random();
...
MMLTSampler::MMLTSampler(MMLTSampler *sampler) : Sampler(Properties()),
...
MMLTSampler::MMLTSampler(Stream *stream, InstanceManager *manager)
...
void MMLTSampler::serialize(Stream *stream, InstanceManager *manager) const {
...
Float MMLTSampler::primarySample(size_t i) {
    while (i >= m_u.size())
...
ref<Sampler> MMLTSampler::clone() {
    ref<MMLTSampler> sampler = new MMLTSampler(this);
...
Float MMLTSampler::next1D() {
    return primarySample(m_sampleIndex++);
}

Point2 MMLTSampler::next2D() {
...
std::string MMLTSampler::toString() const {
    std::ostringstream oss;
    oss << "MMLTSampler[" << endl
        << "  sampleCount = " << m_sampleCount << endl
        << "]";
    return oss.str();
}

MTS_IMPLEMENT_CLASS_S(MMLTSampler, false, Sampler)
MTS_NAMESPACE_END
